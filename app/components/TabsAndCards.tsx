...
const TabsAndCards = () => {
  ...
  useEffect(() => {
    const fetchTabsAndCards = async () => {
      ...
      const reversedTabsWithCards = tabsWithCards.map((tab: TabType) => ({
        ...tab,
        cards: tab.cards.reverse(), // Reverse the order of cards
      }));
  ...
  const handleCardKeyDown = (event: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>, tabIndex: number, cardIndex: number) => {
    if (event.key === 'Enter') {
      updateCardContentHandler(tabIndex, cardIndex);
    }
  };

  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, tabIndex: number, cardIndex: number, field: 'title' | 'text') => {
      const newTabs = [...tabs];
  ...
  return (
    <div className="p-5 bg-gray-800 text-white">
            ...
            <StyledTab
              ...
              label={
                <input
                  ...
                  className="text-white bg-transparent w-24"
                />
              }
            />
          ))}
      ...
      {tabs.map((tab, tabIndex) => (
        <div key={tabIndex} hidden={selectedTab !== tabIndex} className="bg-slate-500 p-5 rounded-b-lg">
              ...
              <Card key={cardIndex} className="w-full mb-2">
                <CardContent className="flex flex-col md:flex-row">
                  <input
                    ...
                    placeholder="Title"
                    className="mr-2 p-1 mb-2 md:mb-10 border border-gray-300 rounded w-full md:w-auto"
                  />
                  <textarea
                    value={card.text}
                    onKeyDown={(e) => handleCardKeyDown(e, tabIndex, cardIndex)}
                    ...
                    placeholder="Text"
                    className="flex-grow p-1 border border-gray-300 rounded w-full md:w-auto"
                    rows={3}
                  />
          ...
          <Button
            ...
            variant="outlined"
            sx={{ color: '#ef4444', borderColor: '#ef4444', marginTop: "10px" }}
          >
            <Delete /> Delete Tab
          </Button>
        </div>
