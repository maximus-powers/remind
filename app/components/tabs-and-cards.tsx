...
import {
  ...
  CardsSkeleton,
  LoadingSpinner,
} from '@/app/components/loading-skeleton';
...
export default function TabsAndCards() {
    ...
    const fetchTabsAndCards = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${baseUrl}/api/data/tabs?userEmail=${userEmail}`
        );
  ...
  const deleteExistingCardHandler = async (
    tabIndex: number,
    cardIndex: number
  ) => {
  ...
  const updateCardContentHandler = async (
    tabIndex: number,
    cardIndex: number
  ) => {
  ...
  return (
    <div className="p-1 bg-white text-black dark:bg-background dark:text-white">
      ...
      {tabs.map((tab, tabIndex) => (
        <div
          key={tab.id}
          className={`${selectedTab === tabIndex ? 'block' : 'hidden'}`}
        >
          <Button
            ...
            disabled={
              isCardLoading &&
              loadingCardAction?.tabIndex === tabIndex &&
              loadingCardAction?.action === 'add'
            }
          >
            {isCardLoading &&
            loadingCardAction?.tabIndex === tabIndex &&
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {tab.cards?.map((card, cardIndex) => (
                <Card
                  key={card.id}
                  className={
                    isCardLoading && loadingCardAction?.cardIndex === cardIndex
                      ? 'opacity-70'
                      : ''
                  }
                >
