...
import {
  updateAudioUrlInDatabase,
  getAudioUrlsFromDatabase,
  updateLastIncludedDate,
} from '../queries';
...
export async function sectionToSpeech(
  section: Awaited<ReturnType<typeof generateScript>>['section1'],
  sectionKey: string,
  rowId: number
) {
...
export async function getSignedUrls(userEmail: string) {
  try {
