...
export default function TabsAndCards() {
    ...
    return (<div className="p-1 bg-white text-black dark:bg-background dark:text-white">
      <div className="flex flex-wrap gap-2 mb-4 justify-center">
        {tabs.map((tab, index) => (<button key={tab.id} onClick={() => setSelectedTab(index)} onDoubleClick={() => setEditTabIndex(index)} className={`${tabColors[index % tabColors.length]} text-white py-2 px-5 rounded-lg transition-all duration-200 ease-in-out ${selectedTab === index ? "font-bold shadow-lg -mb-px" : "opacity-70 hover:opacity-100"}`}>
      ...
      {tabs.map((tab, tabIndex) => {
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {(_a = tab.cards) === null || _a === void 0 ? void 0 : _a.map((card, cardIndex) => (<Card key={card.id}>
                <CardContent className="pt-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Input value={card.title} onKeyDown={(e) => handleCardKeyDown(e, tabIndex, cardIndex)} onBlur={() => updateCardContentHandler(tabIndex, cardIndex)} onChange={(e) => handleInputChange(e, tabIndex, cardIndex, "title")} placeholder="Title" className="flex-grow"/>
                    <Button onClick={() => deleteExistingCardHandler(tabIndex, cardIndex)} variant="ghost" size="icon" className="text-destructive flex-shrink-0">
                      <Trash2 className="h-4 w-4"/>
                    </Button>
                  </div>
                  <Textarea value={card.text} onKeyDown={(e) => handleCardKeyDown(e, tabIndex, cardIndex)} onBlur={() => updateCardContentHandler(tabIndex, cardIndex)} onChange={(e) => handleInputChange(e, tabIndex, cardIndex, "text")} placeholder="Text" rows={3}/>
                </CardContent>
              </Card>))}
          </div>
