...
export async function addNewCard(title, text, tabId) {
  const connection = await mysql.createConnection(dbConfig);
  const [result] = await connection.query(
    'INSERT INTO cards (title, text, tab_id) VALUES (?, ?, ?)',
    [title, text, tabId]
  );
  await connection.end();
...
export async function createNewAudioRow() {
  const connection = await mysql.createConnection(dbConfig);
  const [result] = await connection.query(
    'INSERT INTO audio (was_played, intro, section1, section2, section3, conclusion, script) VALUES (0, NULL, NULL, NULL, NULL, NULL, NULL)'
  );
  await connection.end();
